...
TEST_CASES: list[TestCase] = [
    ...
    TestCase(
        test_id="preference-1",
        query="What music do I like?",
        namespace="personal",
        answer_must_contain=("jazz",),
        expect_sources=True,
        sources_must_contain=("jazz",),
        require_faithfulness=True,
        description="Preference probe, direct form (drift #206 class): a stated personal preference must surface on the direct question. CONTRACT-tier guard: green today; red would mean preference retrieval broke.",
    ),
    TestCase(
        test_id="preference-2",
        query="What do I like to listen to?",
        namespace="personal",
        answer_must_contain=("jazz",),
        expect_sources=True,
        sources_must_contain=("jazz",),
        require_faithfulness=True,
        description="Preference probe, paraphrase form (drift #206 class): no keyword overlap ('listen' vs 'music') — the binding inch the 4B lacks. requires_future_capability: red expected today; the FIRST flip candidate on a model upgrade.",
        requires_future_capability=True,
    ),
    ...
    TestCase(
        test_id="theme-switch-1",
        query="[000] What programming language is Python?",
        namespace="personal",
        conversation_turn=3,
        depends_on="multi-turn-1",
        answer_must_contain_any=("guido", "rossum", "language"),
        answer_must_not_contain=("blue", "sea", "sky", "favorite color"),
        expect_sources=True,
        require_faithfulness=False,
        use_chat_api=True,
        description="Theme switch mid-conversation (drift #164 class): after two favorite-color turns, a DIFFERENT-topic [000] question must not drag the old topic into the answer. requires_future_capability: red expected today (condense carries the old theme); the direct success metric for the condense-isolation fix — flips green when it lands.",
        requires_future_capability=True,
    ),
    TestCase(
