...
| 2026-09-02 | GTX 1650 4GB / 16GB RAM | **faiss migration + incremental indexing verified** (drift #63–64) | **17/17 CONTRACT + 25/30 FUTURE (project record) + 8/9 CHAT on faiss**; RAM peak 5.77 GB (vs ~7 GB memory store). Incremental: document = transaction (embed→upsert→checkpoint), kill-midway resumes via uri+mtime filter, no data loss, no duplicates — proven by kill-midway test invariants. GPU rate measured: ~10 chunks/s at LLM=10 | Serving on faiss; watcher window 600 s is a pause between checkpoints, never a reset; the only residual chat fail remains multi-turn-1 (7B parametric ceiling) |
...
- Corpus ingestion: files >150 KB are split via `scripts/prepare_docs.py` (source: `data/raw_documents/`); CPU embedder rate ~7 chunks/s (3397 chunks in ~500 s, 2026-09-02; ~4 on the first live run, 2026-09-01), GPU ~10 chunks/s (measured, 2026-09-01). Indexing runs on the CPU embedder; there is no separate indexing profile in current use — plain indexing does not involve the LLM (chunker + embedder + vector store only). Serving runs in the normal profile (LLM 20 + CPU embedder). The GPU indexing setup (LLM dropped to 10 layers to free VRAM for the GPU embedder) was a one-time solution for the initial 3400-chunk corpus backfill (2026-09-01), when the CPU embedder could not finish inside the watcher window (drift #60). Store: faiss (RAM peak 5.77 GB on 3400+ chunks, 2026-09-02). Indexing is incremental: a document is a checkpoint — crashes/timeouts resume, never reset (drift #64).
- Chat-path follow-ups: condensation-1 fixed by the condense prompt contract (drift #58); multi-turn-1 is FLAKY on 7B (nondeterministic condensation jitter, 4 red / 2 green; live [d]+'Why?' probe green) and passes on 4B — see the 2026-09-07 rows and the determinism exception above.
