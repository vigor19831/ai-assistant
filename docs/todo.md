...
## 🟠 Блок 2 — Стабильность (неделя 1, предотвращают аварии)
...
[ ] Исправить блокировку event loop: `_trim_history` → `async_count_tokens` | `features/chat/manager.py` вызывает синхронный `count_tokens` (CPU-bound tiktoken/HF) из async метода `_trim_history`. При большой истории весь сервер зависает на секунды — не отвечает на другие запросы. Заменить на `async_count_tokens` (обёртка `asyncio.to_thread`). | `src/ai_assistant/features/chat/manager.py` | `tests/test_chat.py`, `tests/test_e2e.py`
...
## 🟡 Блок 3 — Архитектура и техдолг (неделя 2-3, упрощают поддержку)
...
[ ] Кэшировать повторные вычисления количества токенов | В `generate` → `_truncate_to_fit` → `generate` токены считаются 3+ раза для одного текста. При CJK-тексте — дорого. Без профилирования неясно, является ли это узким местом. Добавить LRU-кэш в рамках одного запроса или сессии только после подтверждения профилем. | `src/ai_assistant/core/pipeline_steps.py`, `src/ai_assistant/features/chat/manager.py` | `tests/test_pipeline.py`, профилирование (`pytest --profile` или `cProfile`)
...
## 🟢 Блок 4 — Документация и инфраструктура (по возможности, предотвращают регрессии)

[ ] Исправить мертвый путь в `[tool.mutmut]` | Указан `src/ai_assistant/pipeline/`, которой не существует. Пайплайн — `core/pipeline.py`. Исправить на `src/ai_assistant/core/pipeline.py`. | `pyproject.toml` | `python -m mutmut run` → не падает с `FileNotFoundError`

[ ] Исправить `README.md`: документировать `vendor/` зависимость | `run_servers.py` ищет модели и бинарники в `vendor/`, но README описывает прямые команды `llama-server`. Добавить секцию про `vendor/` структуру или убрать `run_servers.py` из рекомендуемого пути. | `README.md`, `run_servers.py` (добавить docstring) | Ручная проверка запуска `run_servers.py` без `vendor/`

[ ] Исправить `run_servers.py`: читать порты LLM/Embedder из конфига | Скрипт хардкодит `--port 8080` и `--port 8081`, игнорируя `llm.api_base` и `embedder.api_base` из `config.yaml`. Парсить URL из конфига для определения портов. | `run_servers.py` | Ручный запуск: убедиться что порты из `config.yaml` применяются
...
[ ] Добавить проверку использования всех полей конфигурации | Параметры `mmap`, `mlock`, `n_batch`, `n_ubatch`, `n_gpu_layers` в `LLMConfig` передаются в `LLMConfigData`, но `OpenAICompatibleLLM` (HTTP-клиент) их не использует. Мертвый конфиг врёт пользователю. Добавить smoke-тест, проверяющий отсутствие неиспользуемых конфиг-полей. ⚠️ Удаление полей — breaking change для пользователей с `llama.cpp`. Требует deprecation политики, 2 мажорные версии. | `config.yaml`, `src/ai_assistant/core/config.py` | Новый `tests/test_config_usage.py`

[ ] Добавить архитектурный тест на отсутствие новых `Any` в `core/domain` слоях | AI Rules декларируют строгую типизацию, но новые `Any` могут появляться незаметно. Добавить AST-проверку в CI. Требует CI. Сейчас нет `.github/workflows/`. | `tests/test_architecture.py` | CI, `pytest`

[ ] Добавить архитектурный тест на отсутствие `except Exception` без комментария | Голые перехваты в `pipeline_steps.py`, `handlers.py` — скрывают реальные баги. Сейчас 15+ мест. Должны иметь justification-комментарий или быть зарегистрированы в drift. Требует CI. | `tests/test_architecture.py` | CI

[ ] Добавить архитектурный тест на соответствие `ai_rules` и реального кода | Правила и код расходятся (Jinja2, print, basicConfig, др.). Добавить автоматическую проверку исключений из правил через drift registry. Требует парсера `ai_rules.md` и `drift.md`. Сложно, легко сломаться. | `docs/drift.md`, `tests/test_architecture.py` | CI

[ ] Добавить проверку целостности документации релиза | Проверять в CI наличие файлов и команд, упомянутых в README (`config.yaml`, entrypoint, vendor-структура и т.д.). Фантомные файлы, устаревшие команды — ловятся только ревью. Требует CI. | `README.md`, `.github/workflows/*` | Новый `tests/test_docs_consistency.py`
