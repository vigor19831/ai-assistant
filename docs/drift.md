...
| 66 | 2026-09-03 | prepare_docs round: freshness marker checked part01 only → small as-is sources reprocessed on every run (menu [2] burned ~5 min LLM per press; live verify after fix: 127 ms SKIP); a source grown past the threshold kept its stale as-is copy beside new parts → watcher indexed both versions (_reconcile_outputs in split_file + main); atom seam //2 → //4 (measured 17041 B on 12000 B budget). Lesson: an idempotency marker must be the exact artifact the producer writes; freshness tests must drive the real producer, not hand-crafted outputs (two old tests asserted a world that never exists). Tests 17→20 |
