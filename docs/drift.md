...
| 69 | 2026-09-03 | check_all AST audit: registry keys carried a "src." prefix while real imports are "ai_assistant.*" — absolute imports never matched (false "never imported" on rag/manager.py, grep-disproved); stem fallback marked same-named modules as imported (removed); cycle check counted function-scoped imports as load edges — flagged the deps↔manager cycle that is actually its standard cure (manager imports InitializedAppState at top level, deps.py:39 defers RAGManager into a function; both directions layer-legal); cycles now computed from load-time edges only; unused/dead checks count tests+scripts as callers (get_tasks: test-only callers). Recorded cause "function-scoped imports" was wrong — the collector always visited function bodies |
| 70 | 2026-09-04 | scripts/ + root launchers under ruff+mypy strict (84 lint + 66 type errors at first enable; check_all [3]/[5] lint them via RUFF_TARGETS/MYPY_TARGETS, explicit list §11; check_rag per-file exempt E501/RUF001-3/ASYNC109 per §13.4). FINDING: check_llm.py was broken since the 2026-08-31 #53/#57 purge — still passed top_k/min_p/repeat_penalty/presence_penalty/frequency_penalty (LLM) and n_batch/n_ubatch/mmap/mlock (LLM+embedder) to ConfigData constructors; AttributeError at runtime, 4 days undetected; caught by the FIRST full [5] with scripts included. Also: chat_query dead headers line removed (auth lives on AsyncClient — "chat unauthorized" diagnosis was wrong, grep-proven); decorative api_key params stripped from query_rag/chat_query; stream errors printed instead of swallowed (#55); TypedDict audit registry in check_all (§9). Lessons: (1) isolated `mypy scripts/x.py` resolves ai_assistant imports as Any (ignore_missing_imports) → false zero — src/ai_assistant must always be in the mypy target list; (2) FIND/REPLACE anchors need the exact current file — 5 broken patches in one task; protocol: whole-file exchange |
| 71 | 2026-09-04 | tests/ under ruff (§15 amended per §14): lint debt 372→0 — dead imports/vars, 12 shadowed import redefinitions (arithmetic disproved shadowed TESTS: 971 def + 9 parametrize = 1000 collected, none silent), metrics mock_histogram as-binding moved to the test that uses it, conftest storage fixture return-bug, AdapterError import restored in test_chat failed-stream (test was green via broad Exception catch — NameError masked as adapter failure; argument recorded for future mypy-on-tests decision), SLEEP marker renamed to "sleep: intentional" (noqa-syntax collision, 11 months invisible), ruff format adopted for tests (154/158 E501); Cyrillic/CJK lines = test data (RUF001 noqa×1, ASYNC109 noqa×1 — patched Event.wait signature). Lessons: FIND/REPLACE anchors must come from fresh extraction (ruff line:addr + sed ±8), not memory or ruff render — 12 broken patches across waves; whole-file protocol unworkable at 3k-line test files, region-extractor loop is the working substitute |
| 72 | 2026-09-05 | tests/ under mypy (user decision: non-programmer owner, maximum automation): ~310 findings fixed — 9 fakes promoted to real port implementations (contract defect: fakes duck-typed ports without ABC, found via strict mypy), AppConfig(**dict)→model_validate across tests, NameError-masking import restored, CHAT_NS_PREFIX added to config __all__ (undeclared re-export), MYPY_TEST_FLAGS established (untyped defs, method-assign — mock idiom) with pyproject overrides dead on mypy 1.20.2 (3 config theories disproven); remaining ~80 annotation-cosmetic findings frozen via disabled error codes in MYPY_TEST_FLAGS (arg-type/type-arg/union-attr/call-arg/unused-ignore/name-defined/attr-defined/no-any-return/override) — recorded as deliberate: mock/annotation idiom, not defects; mypy tests line green in [5]. Revisit trigger: tests become a package or a mock-free layer appears. Lessons: 4 sed accidents on one class of edits (tuple comma, bracket-for-comma, eaten argument comma, param rename without body) — all caught by gates (mypy type, SyntaxError, ast-balance); anchors only from fresh extraction; ast.parse after every structural patch; full commands never "..." placeholders; ILLM config-__init__ requires explicit fake constructors; Message not exported from domain.messages (port imports it differently). tests-mypy call must include src/ai_assistant target (mirror of #70 false-zero lesson: without it, port imports resolve as Any and subclass/Any errors flood) |
