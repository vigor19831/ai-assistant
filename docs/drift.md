...
| 132 | 2026-09-16 | Gemma-4-E4B × 0.4.1 replaces Qwen3.5-4B as serving baseline: 16/17 + 9/9 + 29/34 — first-ever 9/9 chat tier, ×2 identical. Live: attribution 4/4, RU held. Qwen same-day re-test: completeness prompt works (retrieval-2 green) but pulls noise on ambiguous queries and leaks indexed facts in no-prefix chat — confirmed non-serving, retained as atomizer. Gemma-archivist rejected: DECISION TEST failed ×3 (paraphrased quote), 12 fabricated dates (guard caught), translit (V5 blind spot, V7 candidate). Atom-batch swap: kill stack → Qwen on 8080 full-GPU → prepare_docs [2] → restart; config untouched. llm_model contract (prepare_docs.py:285): empty = omit field, single-model server — null is the permanent local value |
| 133 | 2026-09-16 | rag_strict.j2: Python example emits the 1991 fact (internal contradiction fixed, trap preserved) + neutral Zenit-E completeness example in a domain absent from bench/corpus; validated cross-model and live. top_k 3→5: condensation-1 recovered, big-1 partial (cat at rank 6+), format-strict-1 red — accepted price (future-tier). ngl: 99 ≡ 999 byte-identical; any partial layout deterministically red on condensation-1 (3/3 full-green vs 5/5 partial-red over 8 runs) — CPU/GPU numeric drift amplified by condensation double-generation; auto-fit present in build but rejected (layout must be deterministic). llama.log empty: launcher -lv 1, no load banner; debug via manual launch (default verbosity 3) |
134 | 2026-09-16 | Iteration B (completeness rule in rag_strict #6) reverted after one run: "all facts about X" pulled the homonym doc into answers (priority-1 snake leak) and flipped condensation-1 to refusal with byte-identical retrieval — rag_strict text joins ngl as a condensation-1 perturbation source (#133 class). Corrected diagnosis: retrieval-2 'guido' is ranking-side — the creator chunk never reaches context for definitional queries; prompt-unfixable. format-strict-1 green under the rule proves completeness works when facts are in context. Rule: a completeness rule must ship with homonym disambiguation.
135 | 2026-09-16 | Archivist e2e gate two-tier: mechanical contract (atoms file, clean non-V5) stays hard red on every model; the verbatim-quote check is model-bound (#128/#132) and degrades to xfail with explanation. Cost: a code-side quoting defect would xfail silently on a non-quote model — covered by validators + canary. Quote tier auto-returns to hard semantics on a quote-capable model.
| 136 | 2026-09-16 | Hybrid stage 1: `ILexicalIndex` port (text-search twin of `IVectorStore`; vector store stays inventory authority) + stdlib BM25 adapter + `LexicalIndexConfigData`; JSON per-namespace disk format v1 — indices are derived data, rebuild by reindex (owner decision 2026-09-16); `LexicalBm25Index._lock` joins the §11.3 exception list (same race as `MemoryVectorStore`). Inert until stages 2-4: no config section, no wiring, no behavior change. Pre-commit fix-ups: (a) ASYNC240 — all five blocking IO calls wrapped in `to_thread`, not just the two ruff saw; (b) rollback in `delete()` was dead code — `_save_locked` wraps OSError into `AdapterError` before the catcher; added the `AdapterError` clause with the same rollback (caught by the failure-path test); (c) upsert-sibling test asserted a term absent from the sibling text — BM25 returns only term-bearing chunks by design; fixed the assertion, not the adapter. Lint lesson: ASYNC240 flags pathlib only on direct `Path()` bindings — Paths returned from helpers escape it; fix the defect class, not the flag reading |
| 137 | 2026-09-16 | Hybrid stage 2: `PipelineData.lexical_index` field (None = pre-hybrid behavior, byte-identical) + `_rrf_fuse`/`_hybrid_fetch` in `pipeline_steps`; `retrieve` and `multi_query_retrieve` RRF-fuse dense+lexical legs, per-leg candidate budget = `fetch_k` (top_k x rerank multiplier); the lexical leg queries the ORIGINAL wording only once (multi_query variations are paraphrases that dilute the exact terms hybrid exists to preserve); tie-break by chunk id ascending (benchmark determinism); lexical failure degrades to dense-only, never an error. `RRF_K=60` in constants (RRF paper value, named constant, no knob). Observability: `rag_hybrid_legs_total` counter + `rag.hybrid` log line (dense/lexical/overlap/fused). Still inert: no wiring fills the field until stage 4. Pre-commit gate catches, all in new code, existing system untouched: (a) import patch duplicated the constants line (find-block missed the pre-existing import); (b) `LEXICAL_INDEX_NOT_PROVIDED` was a speculative orphan constant — removed; a None field is a legal state, not an error; (c) unused imports + long line in the new test; (d) fusion test fixture set top_k=1 so the both-legs candidate never reached fusion — fixture bug, RRF math was right; (e) `with_lexical_index` setter was dead code by construction (managers fill `PipelineData` via constructor kwargs; steps only read) — removed per the orphan rule |
| 138 | 2026-09-16 | Hybrid stage 3: `LexicalIndexConfig` section (provider, index_path — both fields have consumers: factory/adapter/load-on-start; no fusion knobs: `RRF_K` stays a constant, stage-2 decision). Absent section = None = dense-only, the `RerankerConfig` pattern but with a REQUIRED provider when present (reranker's None default is its valid "off"; here "off" is the absent section — a None default would be a third way to say off). No config_version bump: pure addition of an optional section, old configs load unchanged (unlike #121, which removed a field); a bump would be migration machinery with nothing to migrate. Inert: nothing creates the adapter until stage 4b |
| 139 | 2026-09-16 | Hybrid stage 4a (write path): `ILexicalIndex.list_by_filter` added (CORE CHANGE, owner-approved 2026-09-16) — read-only, phantom-namespace-safe, #120 merge semantics; consumers: skip guard, orphan cleanup, clear, delete-by-document (4b). `IndexingManager` dual write: vector first (capacity gate), lexical mirrored; lexical failure recorded not raised (success=False per #113; watcher retry + guard backfill). `index_folder`: dual-store skip guard (drift #93 class — the lexical-contains=>vector-contains invariant makes the lexical side the missing-evidence detector; fresh enable = zero skips = backfill via plain reindex), lexical-first clear/orphan ordering (no resurrection window, #40 class), per-store self-listing clears (retry-convergent), lexical checkpoints to `lexical_index.index_path`. `handlers.py` write sites moved to stage 4b (need `state.lexical_index` — a deps field). All defaults None: production behavior byte-identical until 4b |
| 140 | 2026-09-16 | Hybrid stage 4b (wiring): `init_adapters` creates the lexical adapter iff the config section is present; `AppState`/`InitializedAppState` carry it (cleanup + shutdown-first in lifespan section 3 with None-check per architecture 6.2); lifespan: data dir, startup load (missing file = no-op, corrupt/version = fatal, #73), shutdown save in section 1 without retry wrapper (adapter shutdown saves again idempotently, #43). `RAGManager` + `ChatManager` take `lexical_index` and fill `PipelineData`. handlers: `/rag/index` and save-chat dual-write + mirrored saves; `/rag/delete` mirrors all three selectors — lexical first, each store listing its OWN contents (own-listing catches stale old-id chunks left by a crash between the 4a writes); `_chunk_matches_document` extracted (one match definition); reindex passes the leg to both `index_folder` sites, chat-ns clear mirrored, `_restore` loads both stores. Activation is owner-owned: absent section = byte-identical; enable = add section + restart + POST `/rag/reindex` (guard backfills) + check_rag x2. Gate catches, all in new code, existing system untouched: (a) `LexicalIndexConfigData` sorted before `LLMConfigData` (Lex < LL — third letter); (b) `core/ports/__init__.py` re-export board was never updated in stage 1 — the port file existed, the package name did not; first package-level import exposed it; a new port is TWO lines, the file AND the board; (c) lifespan fixtures fabricated non-None `lexical_index` — MagicMock invents unknown fields as non-None; 9 None-sites fixed by indent-preserving script, 2 live-store sites where the None-scan never visited, and one 4-line anchor matched a twin fixture (first-match replace) — unique-anchor rule applies to tests too; the mocks became truthful, no assertion weakened. Process note: stage-3 draft #138 was lost the same way (drafted, never inserted) — drift drafts must ship as numbered steps |
| 141 | 2026-09-17 | Hybrid activated on live config: check_rag 16/17 + 9/9 + 29/34 — level-identical to baseline; set composition changed (format-strict-1 green via the lexical leg; big-1 jazz/cat pushed deeper; 'guido' unchanged — reranker verdict on definitional queries, not retrieval). Peak RAM 5.86 GB vs 5.39 baseline (+0.5 GB lexical texts in memory) — the measured price. Fixture hunt closed: MagicMock invents unknown CONFIG sections exactly as it invents unknown state fields — the watcher test named every config field except `lexical_index` and `mkdir` on the mock path created `./MagicMock` in the repo root on every run; caught by an autouse `/tmp` fixture failing the creating test. Rule: every mock touching lifespan must name BOTH state AND config fields explicitly — a MagicMock is only honest about what it is told. Formatting rule for drift rows: underscored/parenthesized identifiers are wrapped in backticks — bare pairs italicize the whole table below (bit us in #136) |
...
| condensation-1 verdict flips with GPU offload layout (measured, #133): full offload green 3/3, partial (incl. CPU) red 5/5 — deterministic per layout, not a flake; 7B-era multi-turn-1 jitter retired with 7B (#129). Condensed query logged at DEBUG only | serving layout changes (ngl, engine update) or a surprising condensation red | verify offload layout first; raise condensed-query log to INFO (§2.7) | flake rate grows on the current serving pair, or the prefix is used live | re-run before classifying a surprising red; raise the condensed-query log to INFO (§2.7) |
