...
| AST-audit function-scoped imports | check_all orphan-detection misses imports inside function bodies (deps.py:39, indexing.py:245) → false "never imported" on rag/manager.py, false "unused" on get_tasks (grep-disproved 2026-09-03). Until fixed: every orphan verdict requires grep confirmation (ai_rules §0). Fix trigger: next false positive reaching a commit |
