...
| 165 | 2026-09-21 | OWNER BACKLOG: a separate max_tokens for plain (no-prefix) chat answers -- the shared llm.max_tokens=768 truncates long off-RAG answers mid-sentence (live: anatomy lecture cut at "**Па"); RAG answers fit fine. Design: chat.max_tokens_plain (optional, falls back to llm.max_tokens); plain path uses it, RAG path unchanged. Implementation next session (config + chat manager sampling override + tests); NOT a hotfix -- RAG answers unaffected |
