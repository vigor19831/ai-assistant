...
| 146 | 2026-09-17 | Lexical mirror backfill at startup (CORE CHANGE: new `IVectorStore.list_chunks`, both adapters): every vector namespace with chunks missing or stale in the lexical index (diff by chunk id + last_modified) is re-fed from stored texts — the embedder is never called; enabling hybrid no longer re-embeds the corpus (runbook #140 amended: enable = section + restart; full reindex stays for embedder model changes only, #60); source-granular upsert converges partial mirrors and drops stale same-source siblings; save failure = WARNING, self-heals next startup; chat namespaces covered; no config, no endpoint; no. 144 intentionally skipped (16/17-limit draft superseded by the actual 17/17); `list_chunks` is the codebase's one exact duplicate (both store adapters) — adjudicated: 2 sites is below the >=3 refactoring threshold (arch 7.1), sharing would cost a Protocol + a new file for 8 lines (2.1/2.6); behavior parity pinned by a contract test; check_all's duplicate audit gained a name-scoped adjudicated-exceptions registry (ADJUDICATED_DUPLICATES, the MYPY_TEST_FLAGS pattern #72): it exempts THIS def between THESE two files only — a new accidental duplicate between the same files is still flagged, a rename re-flags the entry for review |
| 147 | 2026-09-17 | Hardware Ceiling Log: CPU embed rate re-measured live — ~8.7 avg / 7.6-10 per doc (bge-m3, engine 0.4.1, 271 chunks in 6 single-batch docs, chunk 512) vs ~4-7 (2026-09-01/02, pre-0.4.1); gain unattributed, single session — plan bulk loads with a 1.5x margin. Consequence: the drift #60 GPU profile switch for large backfills buys little at these rates (~135K chunks ≈ 4-5 h CPU); prepare_docs docstring cites the re-measurement, split threshold unchanged (150 KB still fits the 600 s window); docs-only, no behavior change |
| 148 | 2026-09-17 | Coverage iteration (tests only, no behavior change): 21 tests closing integrity-contract gaps — FAISS load guards (missing/corrupt/mismatched index pairs fail loudly, never a silent empty index), delete/upsert save-failure rollbacks (both stores), max_chunks overflow on the faiss add and memory upsert paths, orphaned half-pairs never listed as namespaces, list_namespaces survives an unlistable path, backfill WARNING branches (#146 debt); suite 1183→1204 |
| 149 | 2026-09-17 | Date campaign stage 1 (variant B, no core change): prepare_docs normalizes session dates at split time into machine-form markers `[Speaker, YYYY-MM-DD]` (year from the SOURCE mtime; month tables stay inside the script — src stays digits-only) and split outputs inherit the source mtime (a re-split neither moves years nor re-announces documents to the watcher); indexing extracts `doc_date` "YYYY-MM" (last marker wins; notes: full digital date as the first line only) into the existing `ChunkMetadata.custom` — no disk format change, no config_version bump; no honest date -> empty value; retrieval behavior unchanged (nobody reads doc_date yet); re-baselining required: corpus re-split + check_rag ×2 + live attribution mini-session (#130 protocol — the marker format changed, measured territory #124) |
| 151 | 2026-09-17 | Date campaign stage 2 (CORE CHANGE: optional `date_filter` param on IVectorStore.search + ILexicalIndex.search, all three adapters updated; DateFilter in domain, rides PipelineConfig): pre-retrieval filtering in BOTH legs (one leg unfiltered would leak foreign months through RRF); undated chunks excluded when the filter is active; faiss widens the fetch x4 before filtering; the date phrase is parsed mechanically (query_parser.parse_date_phrase) and STAYS in the query text; month dictionary lives in config rag.date_month_names (variant 4b — src stays language-agnostic), pure optional addition, no config_version bump (#138); DOC_DATE_KEY moved to core.constants (adapters must not import features); no phrase / no dictionary = byte-identical pre-stage-2 path; stage 3 (if ever) = benchmark tier for date queries, owner-gated |
