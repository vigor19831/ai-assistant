...
| 78 | 2026-09-06 | decision-guard campaign closed at benchmark maxima: 17/17 + 9/9 (first ever, multi-turn-1 green ×3). Live eval exposes a see-saw the benchmark cannot: rule 12 ± fact-examples oscillates on the advice-as-answer class (4 swings, both directions: watch-advice quoted as decision-answer; router-recommendation falsely refused with its atom in Sources). Stable floor: validator auto-demotions (zero manual edits), autodom/cream/F6722 traps, atom body attribution. Lesson: benchmark-green ≠ live-green on 7B; cures = 14B class or Golden Set, not a 5th prompt iteration. Validator ChatGPT-format-bound — next: DeepSeek probe |
| 79 | 2026-09-07 | qwen 3.5 4B re-validated under campaign prompts; owner decision: stay on 4B as daily driver, waiting for next generation. Atom layer: best ever (RU atoms, zero fabricated decisions, 9.5 min vs qwen 2.5 7B's 23 — ×2.4); answer layer: all 4 decision-traps honest (see-saw absent — boundary was model-width, not prompt), EN answers on RU queries persist (~50%, drift #67); RU-repeat («на русском») recovers content-identical RU answers in 6/6 cycles, cost = manual re-ask + ~10 s; user accepts as permanent tax. 4B-forever is a waiting position, not dogma: revisit at next model generation (14B-class on affordable VRAM — Ceiling Log trigger) or when RU-answers become critical. Hybrid (4B archivist / 7B chat) remains the fallback, one config swap away |
