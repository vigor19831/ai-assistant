...
| max_chunks overflow surfaces mid-indexing as AdapterError (#48 contract) — functional but opaque; no pre-flight check tells the operator BEFORE a long run starts | a namespace approaches ~80K chunks | pre-flight chunk-count check with a human-readable refusal in index_folder (stage 1) |
