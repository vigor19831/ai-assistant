...
| 119 | 2026-09-13 | Reindex interruption symmetry: the 4h timeout branch restores affected namespaces from disk exactly like cancellation — `_restore_reindex_namespaces` (module-level in handlers.py) is the single restore path for both branches; memory must not stay ahead of the last durable state after any interrupted run. Note: `asyncio.sleep()` allowed in `tests/test_incremental_read.py` only (pyproject per-file ignore) — the sleep IS the tested mechanism (hanging save/index past a timeout); every other test file keeps the §15 no-sleep rule |
| 120 | 2026-09-14 | `list_by_filter` typed-wins parity: faiss merges custom keys first, typed fields overwrite them (memory already did this in add/upsert) — a same-named custom key can no longer shadow `source`/`source_uri` in one store but not the other (#15); the no-op `except Exception: raise` in faiss `_save_unlocked` removed — the finally-cleanup was the only useful part |
