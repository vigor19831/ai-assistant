...
| prepare_docs: a document moved out of atomize/ back to the root keeps its atoms file — atoms stop refreshing but are never removed; a stale atoms file keeps serving retrieval | first observed stale answer from a demoted document | extend the planned deletion-cascade reconcile to demotions — owner decision |
