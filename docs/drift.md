...
| 159 | 2026-09-21 | Live session (40+ probes, RU corpus, Gemma, JSON-migrated): F1 advice-as-action DROPPED by JSON migration (3/3 red on .md unlabeled -> green with structural roles); F2 recommendation-as-decision survived 3 conveyors — model-independent residual, retest on every model upgrade; F5 closed twice: v1 (collapsed-paste noise, JSON migration) and v2 (condense topic-substitution 'зрам'->'снапшот'/'Tuxedo', pre-retrieval corruption — isolated by empty-chat test, logs: top_score 0.97-0.99 on corrupted queries); best live attribution ever (speaker+date cited in answers). Testset lesson: a re-exported chat is a new corpus — grep-verify expectations before classifying a red (two false 'fabrication' verdicts corrected) |
| 160 | 2026-09-20 | Corpus migrated to chat-JSON (SaveAI format): _chat_json_to_markdown in prepare_docs (role+created_at+displayModel -> [Speaker, date] markers; non-text skipped+counted; non-chat JSON -> loud [SKIP-JSON]); artifacts land as .md; reconcile/orphan/freshness json-aware. Two patch bugs caught same session (needs_processing fallthrough, _find_vanished part-stem) — rule: every freshness/orphan change ships with its "stale marker" test; multi-part test edits go by-name scripts, not FIND/REPLACE (glued-tail lesson, 5 iterations) |
| 161 | 2026-09-21 | condense_question.j2 rewrite (4 iterations, #78 stop-rule): REQUIRED pronoun resolution + topic-is-final + MAY-enrich-same-topic + NEVER-different-topic; 3 EN examples incl. the snapshot-distractor. CLOSED live: colloquical named-topic queries no longer substituted (stress test: fresh distractor topic in history, held); live pronoun resolution green ('какой у него запас хода' -> 40h). PRICE: condensation-1 -> known limitation (correct condensed query, abstract form lacks retrieval signal for the bench doc; Gemma ignores MAY-enrich; live analogue passes). Bench 28/34 vs 29/34 canon — the traded test for a closed live defect |
| 162 | 2026-09-21 | .md chat-export cleaner retired from prepare_docs (~250 lines: _clean_chat_export, _SPEAKER_MARKERS, _NOISE_LINES, _strip_markdown_noise, source_year): chats arrive as JSON only; .md/.txt = plain documents, byte-identical pass-through (contract test added). _USER_BLOCK_RE/_validate_decisions kept (atoms consumer); atoms reactivation note: decision validator needs a JSON rewrite (contents[].content with role=user). run_scripts.py menu text updated |
| 163 | 2026-09-21 | LANGUAGE RULE (owner decision): src/ is English-only INCLUDING .j2 prompt templates; Cyrillic stays in tests/fixtures, scripts' language tables, owner config. The condense RU example was translated EN; the RU colloquial condensation pattern is validated by live probes only. Any future src/ code needing owner-language markers (e.g. marker converter in src/) requires an explicit drift entry as a rule exception |
