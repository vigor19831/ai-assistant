...
> date campaign, docs sync) compacted the same way).
>
...
| 142 | 2026-09-17 | Comment audit (884 lines): 4 defects fixed, 876/884 load-bearing — density healthy; classes confirmed (drift links / decisions-with-cost / spec blocks) |
| 143 | 2026-09-17 | rag.condense → INFO, stream refusal logs chunks_used=0 (#50 parity) — log-only; gate catch: UP032 (the rules bind the AI too) |
| 145 | 2026-09-17 | rag_strict synthesis (owner-merged): 17/17 + 9/9 + 29/34 — first full contract+chat on Gemma; the EXAMPLE is the only 'guido' catalyst; priority-1 red by design tension (see FUTURE RISKS); bench-case examples unneutralized (optional future) |
| 146 | 2026-09-17 | Lexical mirror backfill at startup (CORE CHANGE: `IVectorStore.list_chunks`): missing/stale mirror chunks re-fed from stored texts, embedder never called — enabling hybrid no longer re-embeds the corpus (#60 runbook: full reindex only for embedder model changes); adjudicated duplicate via check_all's ADJUDICATED_DUPLICATES registry (name-scoped); #144 intentionally skipped |
| 147 | 2026-09-17 | CPU embed rate re-measured: ~8.7 avg (0.4.1) vs ~4-7 (pre-0.4.1) — #60 GPU switch buys little (~135K chunks ≈ 4-5 h CPU); docs-only |
| 148 | 2026-09-17 | Coverage +21 tests: FAISS load guards, save-failure rollbacks, overflow paths, backfill WARNING branches — integrity contracts pinned; suite 1183→1204 |
| 149 | 2026-09-17 | Date campaign stage 1 (variant B): machine-form markers at split time (year from SOURCE mtime; tables stay in the script — src digits-only), split outputs inherit source mtime, `doc_date` YYYY-MM into `ChunkMetadata.custom` (last marker wins; no honest date → empty); re-baselined (#130) |
| 151 | 2026-09-18 | Date campaign stage 2 (CORE CHANGE: optional `date_filter` on both ports, 3 adapters): pre-retrieval frame in BOTH legs, undated excluded while active, phrase parsed mechanically and STAYS in the text, language data in yaml (4b), no phrase = byte-identical old path; live-verified: "что я решал в июле" → July doc only |
| 152 | 2026-09-18 | Docs sync: owner requirements codified as the top block of ai_rules.md (9 requirements + standing facts: sacred data, target scale, deployment profile; Cyrillic-as-data; the core-change gate stays hard — owner-authorized, never AI-initiated); architecture §1 + portability/data-sacred/profile, §2.9 date markers + doc_date, §13 invariant #8 (date frame = query scope), §14 live RAM point + owner-profile note; README date-filter feature; ai_rules §14 retires ai_rules_proposed.md; anchor-freshness rule (§9); Cyrillic-fixture/ClassVar rules (§15) |
| 154 | 2026-09-18 | Hardcode audit (fresh session, owner-framed definition): CLEAN — 0 violations (~16 eternal constants legit, ~20 pydantic defaults yaml-overridden). Follow-ups shipped: cloud-model defaults (gpt-4o-mini / text-embedding-3-small / rerank-multilingual-v3.0) → empty strings + startup guard `_check_explicit_models` (remote provider must name its model — llama-server ignores the field, so empty alone was not a loud failure; mock providers exempt; archivist #132 pattern untouched); config.example.yaml + date_prepositions (a pure optional addition — no config_version bump per #138; the 3→4 bump was #121's); the api_base follow-up shipped the same day: cloud endpoint defaults (api.openai.com, api.cohere.com) → empty strings in BOTH config layers (Pydantic + domain dataclasses) with the same startup guard — a forgotten api_base with an OPENAI_API_KEY in the env spent real money silently; mock providers exempt; the OPENAI_API_KEY env fallback in adapters stays (explicit opt-in) but can no longer combine with an implicit cloud URL |
| 155 | 2026-09-18 | Hardcode audit round 2 (two independent audit opinions reconciled): the config-freeze rule (§11.2) UPHELD against 8 extraction proposals — zero live cases for watcher, SSE, condense and status knobs, each would be an owner-less yaml line; style debt paid: seven bare literals named (LARGE_CONTEXT_THRESHOLD, TOKEN_MARGIN_CAP, MULTI_QUERY_VARIATIONS synced with multi_query.j2, TOP_K_MAX, YEAR_MIN, YEAR_MAX, HEALTH_CHECK_TIMEOUT) plus justification comments on the INDEX_IO, ADAPTER_SHUTDOWN and REINDEX timeouts and on `_HISTORY_TOKEN_OVERHEAD`; file encodings moved to rag.file_encodings (owner language data, the audit's single real finding — the latin-1 tail silently mojibakes koi8-r; optional addition, no bump per #138); example.yaml behavior wording made honest (the 60 s poll is a constant, not a setting); no behavior change with the default config |
| 156 | 2026-09-18 | Test mannequin factory campaign (8 commits): conftest factories (`real_state` = real `init_adapters` over mock providers; shell, chunk and test-client builders) plus per-file local helpers replaced hand-built test assemblies in 6 test files; 2 more audited healthy, skipped; `build_mock_state` kept for mock-path consumers, synced with `init_adapters` kwargs — the #151 wiring-drift class closed for every consumer; fabricated-mock coverage exposed (lifespan 80→76% honest); protocol lessons: Replace All pairs state expected counts, single pairs carry unique context anchors, full-file replace is worded "replace the entire file", "not found" on re-apply means already applied |
| 157 | 2026-09-18 | Test hygiene closeout: `chat_storage_adapter` fixture's `:memory:` was doubly broken (per-call connections kill the schema AND grep proved zero consumers ever) — five dead parametrized contract fixtures removed (~80 lines; template `vector_store_adapter` stays live in the list_chunks parity test); marker registration verified in pyproject with `--strict-markers` — false alarm; empty TestChatHistoryTrimming dropped; lexical shutdown persist (lifespan stage 1b) covered honestly — 76%→80%; lesson: pytest fixtures are invisible to the AST dead-code audit, grep by fixture name is the tool |
| 158 | 2026-09-19 | Verified backup feature (CORE CHANGE, owner-authorized: optional `backup.target_dir` section in AppConfig — ArchivistConfig pattern, no version bump per #138): `scripts/backup.py` copies the three non-rebuildable assets (config.yaml, `data/raw_documents/`, the chat-history db) — sqlite3 backup API for the live WAL db, every copy self-verified (file count, bytes, integrity_check, YAML re-parse), exit 1 on any mismatch; timestamped folders, never auto-deleted; target must be outside the project (disk-death guard); 3 e2e tests in test_smoke on a tmp mini-project; run_scripts: explicit `_SCRIPTS` menu order replaces glob discovery (§11 — glob put backup first); check_all: backup.py added to MYPY_TARGETS; config.example.yaml documents the section; first live backup verified 2026-09-19; lesson: a new script touches three lists — run_scripts `_SCRIPTS`, check_all MYPY_TARGETS, config.example.yaml |
| 159 | 2026-09-21 | Live session (40+ probes, RU corpus, Gemma, JSON-migrated): F1 advice-as-action CLOSED by JSON migration (3/3 red on unlabeled .md → green with structural roles); F2 advice-as-decision is model-independent (3 conveyors) — retest B2 on every model upgrade; F5-v2 condense topic-substitution (pre-retrieval corruption, isolated by empty-chat test) CLOSED by #161; best live attribution ever (speaker+date cited). Rule: a re-exported chat is a new corpus — grep-verify testset expectations before classifying a red |
| 160 | 2026-09-20 | Corpus migrated to chat-JSON (SaveAI format): _chat_json_to_markdown (role/created_at/displayModel → [Speaker, date] markers, non-text skipped+counted, non-chat → loud [SKIP-JSON]); .md artifacts; reconcile/orphan/freshness json-aware; two patch bugs caught same session (needs_processing fallthrough, _find_vanished part-stem). Rules: freshness/orphan changes ship with their stale-marker test; multi-part test edits go by-name scripts, not FIND/REPLACE (glued-tail lesson) |
| 161 | 2026-09-21 | condense_question.j2 rewrite (4 iterations, #78 stop): REQUIRED pronoun resolution + topic-is-final + MAY-enrich same-topic + NEVER different-topic; snapshot-distractor example. CLOSED live: colloquial named-topic queries no longer substituted (fresh-distractor stress test held); live pronoun resolution green. PRICE: condensation-1 → known limitation (condensed query correct, lacks retrieval signal for the bench doc; live analogue passes); bench 28/34 vs 29/34 canon |
| 162 | 2026-09-21 | .md chat-export cleaner retired from prepare_docs (~250 lines); chats arrive as JSON only, .md/.txt = plain documents, byte-identical pass-through (contract test). _USER_BLOCK_RE/_validate_decisions kept (atoms); atoms reactivation requires a JSON decision-validator rewrite (contents[].content, role=user) |
| 163 | 2026-09-21 | LANGUAGE RULE (owner decision): src/ English-only INCLUDING .j2 prompt templates; Cyrillic only in tests/fixtures, scripts' language tables, owner config. RU colloquial condensation is validated by live probes only; any future src/ owner-language markers need an explicit drift entry |
| 164 | 2026-09-21 | Condense residual class (1 live case): a colloquial topic word phonetically close to a recent-history word gets "autocorrected" INTO it (zram->zrachok/pupil, twice, after an off-RAG answer poisoned the history; fresh-chat test: correct 7.5 GB answer). Distinct from F5-v2 (theme substitution): the model reads the named topic as a typo of the history word. Off-RAG turns in a [d]-chat pollute condensation — root fix scheduled (off-RAG answers excluded from the condense context, features/chat). Trigger to revisit the prompt clause: 3rd case. ...root fix SHIPPED: both roles filtered (user prefix + assistant Sources marker), live trap green |
| 165 | 2026-09-21 | OWNER BACKLOG → implemented as #176 (chat.max_tokens_plain): superseded, see #176 |
| 166 | 2026-09-21 | context_build truncation removed: ground-truth docs ship IN FULL, loud 80k size tripwire; drift History tail + FUTURE RISKS were silently lost in generated contexts (ai_rules §0 synced) |
| 167 | 2026-09-21 | run_servers hardening: venv check before config load, default-key + non-loopback guard, -ngl single-source, honest readiness (foreign port, exit codes), Ctrl+C forces stop, kill_main aligned with kill.py |
| 168 | 2026-09-21 | check_rag hardening: tier counters upfront, timeout 65 s (SLA live), per-occurrence negation window, stream retry parity, pipeline errors scored, monitor into the log; corpus: EN apple mirror + constraint fixes; re-baseline expected |
| 169 | 2026-09-21 | prepare_docs, batch 4: grown-json cured both sides (stale as-is copy + symmetric freshness); post-atomization validation resolves through the mirror root; atoms artifacts use _out_suffix (.md for json); --full layer separation (fresh atoms + stale split = re-split only); _QUOTE_MAX named; flags validated before the destructive reconcile |
| 170 | 2026-09-22 | Bench re-baseline: atoms-undated-1 GREEN (FUTURE RISKS row closed); edge-2 red = top-5 width on broad queries; canon 16/16 + 8/9 + 29/35, 6 known limitations |
| 171 | 2026-09-22 | kill.py: probe fallbacks guarded individually, killall counted, honest exit code (held ports = 1), _kill_pid verifies returncodes; aligned with run_servers.kill_main |
| 172 | 2026-09-22 | Small fixes, 8 scripts (backup, check_llm, download_tokenizers, clean_cache, structure, run_scripts, check_all, open_shell): honest counters/exit codes, dead code removed, wildcard/Windows fixes — full list in git |
| 173 | 2026-09-22 | mutmut_check: relaunch flag leak fixed, Windows branch added; "all killed" verdict retracted (mutmut 2.x exit codes don't encode survivors) — measured statements only |
| 174 | 2026-09-22 | Docs batch: canonical Apache-2.0 LICENSE, NOTICE (bundled ui libs), pyproject package-data (non-editable installs), architecture/ai_rules synced to #160/#162/#163, FUTURE RISKS updated |
| 175 | 2026-09-22 | Final pass: comment audit over the wave diff (all load-bearing), _REASON_MAX named, conflict-ru-1 word list tightened, context_build docstring, NOTICE resolved, _DOC_SIZE_WARNING=80000 ratified, Qwen-heir profile synced |
| 176 | 2026-09-22 | CORE CHANGE: ChatConfig.max_tokens_plain — optional plain-chat generation budget (RAG path untouched, explicit values win, stream shares it); cured mid-sentence off-RAG truncation |
| 177 | 2026-09-22 | Chat UI attach audit: WORKS client-side (10 files / 5 MB / 50K chars → text into the message; initial "no backend" was an audit error); limitation = the 8K context window for large docs — FUTURE RISKS row |
| 178 | 2026-09-22 | admin.py state typed as InitializedAppState (dead None-checks removed); main.py version from package metadata (was hardcoded, OpenAPI lied) |
| 179 | 2026-09-22 | Final gate, bench half: ×2 identical 16/16 + 9/9 + 29/35 (post-ecb8050 canon); condensation-1 GREEN (fragile, not trusted), format-strict-1 red (width class) |
| 180 | 2026-09-22 | Final gate, live half (30+ probes): core green; NEW CLASS premise-compliance red (model accepts an absent presupposition); width reds at multi-doc scale; genitive date forms unparsed (yaml data); EN refusals → #186 |
| 181 | 2026-09-22 | Prompt sync guard caught a bulk-pass defect (rag_simple.j2 taught the reserved refusal phrase — rephrased); lesson: a bulk template pass commits only after a green check_all |
| 182 | 2026-09-23 | rag_strict.j2 RU markers (#163-required authorization): rule 12 verb list, rule 13 «Дата не указана», RU-question examples — bench-validated attribution/date data |
| 183 | 2026-09-23 | Live audit: lexical_index missing from the qwen profile (silent section loss — config-liveness can't see absence); rule: a profile diff contains ONLY model sections; gemma never affected |
| 184 | 2026-09-23 | Qwen-heir serving gate FAIL both configs (dense-only 14/16, hybrid 15/16): over-synthesis, over-refusal at perfect retrieval, narrated refusals leak distractors — non-serving |
| 185 | 2026-09-23 | Gemma-4-E4B QAT adopted as serving baseline: ×2 identical 16/16 + 9/9 + 30/35, strictly better than bartowski (format-strict-1 green in RU); premise-compliance + topic-carry green live; VRAM 3030 MB; rollback profiles retained |
| 186 | 2026-09-23 | CORE CHANGE: bilingual refusal contract — rag.refusal_phrase_local (owner yaml data, src ASCII) rendered into rag_strict and matched on both entry paths; preamble-before-closing permitted; injection refusal stays EN |
| 187 | 2026-09-23 | Bilingual re-baseline: deps wiring gap caught by missing-ru-2 (its designed job); negation window fixed both-directions (bench self-contradiction); trap-2 → one rule-2 iteration (#78); conflict-ru-1 green; canon 16/16 + 9/9 + 31/36 |
| 188 | 2026-09-24 | Bare-day date frame (CORE CHANGE, owner-authorized): a day + month phrase with no preposition ("что мы обсуждали 5 июля") now activates the month frame — day digits anchor the match, a bare month name alone never activates; shape precedence digital > prepositional > bare day; month-only resolution, doc_date granularity unchanged; query-time only, no reindex, no config change; closes the live class from the 2026-09-23 real-corpus probe (router-doc URL date answered as a July fact, test real-date-july5) |
| 190 | 2026-09-24 | Date campaign stage 3 (CORE CHANGE, owner-authorized): DateFilter.day — "что мы обсуждали 5 июля 2026" now filters to the exact day, removing the ranking race inside the month (live: the 5-July shoes doc lost the semantic top to longer 10/17/28-July chats, honest refusal on top_score=0.0158); doc_date stored as full YYYY-MM-DD (legacy YYYY-MM still matches month frames, never day frames — the chunk must state the day it is matched by); day requires year, bare day without year degrades to a month frame; sliding inheritance: a markerless chunk inherits the last seen marker's date (899/1093 chunks were None — assistant-answer tails), plain docs stay undated byte-identically; one reindex closes the campaign |
| 191 | 2026-09-24 | FAISS adaptive fetch widening (drift #190 follow-up): a date filter left fewer than top_k results after the widened (x4) fetch — the fat 2026-09-04 day (160 chunks) sat outside the semantic top-40 and the query refused; the fetch now escalates x4 -> x100 -> full scan until top_k filtered results or the whole index is scanned (flat-index pre-filter equivalent, milliseconds at project scale); dedup via seen_ids across passes; BM25 leg unaffected (it filters before scoring); contract note (owner decision): a day requires an explicit year — "4 сентября" stays a month frame (wide, semantic race), "4 сентября 2026" is the day frame; the current-year default was rejected as a drift-#80/#81-class guess.|
| 192 | 2026-09-24 | multi_query abstraction bridge: canon held, target probe NOT met — variations fired (metrics=3), failure was chunk dilution, not the prompt; routed to #193 |
| 193 | 2026-09-24 | Speaker-aware chunking v1/v2 REVERTED: mechanically sound (0.29→0.43, chunk retrieved), the 4B model declined to bind — slicing cannot fix a comprehension ceiling; conversation-shaping belongs to ingestion; revisit via atoms/14B (POLICY #92) |
| 196 | 2026-09-24 | Atoms pilot on the live corpus: Gemma-4 passed (clean attribution, zero inversions), Qwen3.5 glossier but hallucinated a JSON-"fact" — CAUGHT by V5; live probes green, "что я говорил про воск" retrieved the owner's first-person line — the my-voice class IS reachable |
| 198 | 2026-09-24 | Serving-model comparison: Qwen3.5-4B fails the bilingual refusal contract, typo-1, bloats chat answers — Gemma-4 restored as serving, Qwen kept as atomizer (#132 pattern) |
| 199 | 2026-09-24 | Hardware ceiling reaffirmed by owner measurement: 8B-class degrades latency AND quality on this laptop — ceiling is 7B, practically the current 4-8B Gemma; roadmap is atoms+data-only |
...
| 204 | 2026-09-25 | make_atoms connection error humanized (owner request, same class as #203): httpx.ConnectError now prints a one-line diagnosis (the LLM address from config, "is it running?", restart cleanly, nothing lost) instead of a 40-line httpcore traceback — the live case: morning run without the stack started, read as a code defect; exit 1 for scripting; only ConnectError is caught, timeouts/HTTP errors keep their original handling |
| 205 | 2026-09-25 | ARCHIVIST_PROMPT first-person preference rule (owner-authorized, one iteration #78): user's own liking/choice statements become Facts in the DIRECT form («Пользователь предпочитает X») — never rephrased into project terms; live-proven: the preference atom written in direct form; Cyrillic prompt examples normalized (translit retired); strengthens never overrides body-phrasing (assistant advice keeps attribution) |
| 206 | 2026-09-25 | Pickup probe CLOSED as model-bound, FINAL: the direct-form atom («Пользователь предпочитает пикап Isuzu D-Max», manually verbatim-edited) reached the answer context — the 4B model STILL replied with the advice list ("Ваш выбор... включает Hilux, Ranger, D-Max"), closest approach ever ("Ваш выбор" appeared) but the binding never completes; every data-side layer proven fixed across 5 attempts (chunk isolation, titles, headers, direct atom, retrieval — Sources carried the atom each time). Reopen triggers ONLY: (a) serving model upgrade, (b) atoms on a stronger model per POLICY #92. No further data-side work can move this |
...
| Date-phrase declension: the month table holds nominative/genitive/prepositional after the 2026-09-23 compression | a live date query in an unparsed case | extend the yaml table (data, no code) |
