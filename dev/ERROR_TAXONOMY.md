## 🧨 ERROR TAXONOMY
> Auto-generated from source code. Updated: 2026-06-05 10:59 UTC
...
| `api.deps` | `RuntimeError` | LLM adapter failed to initialize | High | 104 |
| `api.deps` | `RuntimeError` | Embedder adapter failed to initialize | High | 106 |
| `api.deps` | `RuntimeError` | Vector store adapter failed to initialize | High | 108 |
| `api.deps` | `RuntimeError` | Storage adapter failed to initialize | High | 110 |
| `api.deps` | `RuntimeError` | Chunker adapter failed to initialize | High | 112 |
| `api.deps` | `RuntimeError` | Chat manager failed to initialize | High | 114 |
| `api.deps` | `RuntimeError` | Pipeline failed to initialize | High | 202 |
| `api.deps` | `RuntimeError` | State not initialized | High | 231 |
...
| `dev.tests.test_api_deps` | `ValueError` | Broken config | High | 304 |
...
| `features.chat.manager` | `AdapterError` | LLM stream failed: {...} | High | 364 |
| `features.rag.handlers` | `HTTPException` | Unknown error | High | 246 |
...
| `api.deps` | `ValueError` | _logger.exception( | Medium | 150 |
| `api.deps` | `ValueError/ImportError` | _logger.exception( | Medium | 158 |
| `api.lifespan` | `Exception` | logger.exception("Index load failed on startup") | Medium | 67 |
| `api.lifespan` | `Exception` | logger.exception("Index save failed") | Medium | 118 |
| `api.lifespan` | `Exception` | logger.exception("%s shutdown failed", name) | Medium | 130 |
| `api.lifespan` | `OSError` | logger.warning("Failed to write PID file: %s", exc) | Medium | 74 |
| `api.lifespan` | `OSError` | logger.warning("Failed to remove PID file: %s", exc) | Medium | 89 |
| `api.lifespan` | `TimeoutError` | logger.warning("Index save timed out: %s/%s", index_path, ns | Medium | 115 |
...
| `core.pipeline_steps` | `AdapterError` | _logger.exception("LLM unavailable", extra={"trace_id": data | Medium | 333 |
| `core.pipeline_steps` | `AdapterError` | _logger.exception( | Medium | 392 |
| `core.pipeline_steps` | `Exception` | _logger.exception("embed_query failed", extra={"trace_id": d | Medium | 120 |
| `core.pipeline_steps` | `Exception` | _logger.exception("retrieve failed", extra={"trace_id": data | Medium | 157 |
| `core.pipeline_steps` | `Exception` | _logger.exception("rerank failed", extra={"trace_id": data.t | Medium | 231 |
| `core.pipeline_steps` | `Exception` | prompt = _build_fallback_prompt() | Medium | 286 |
| `core.pipeline_steps` | `Exception` | _logger.exception( | Medium | 338 |
| `core.pipeline_steps` | `Exception` | content = f"Error: {e}" | Medium | 380 |
...
| `dev.tests.test_api_deps` | `ImportError` | sqlite3 not available | Medium | 284 |
...
| `features.chat.manager` | `AdapterError` | raise | Medium | 256 |
| `features.chat.manager` | `Exception` | logger.warning( | Medium | 233 |
| `features.chat.manager` | `Exception` | logger.warning("History load failed: %s", exc) | Medium | 251 |
| `features.chat.manager` | `Exception` | logger.error( | Medium | 258 |
| `features.chat.manager` | `Exception` | logger.warning("History save failed: %s", exc) | Medium | 298 |
| `features.chat.manager` | `Exception` | history = ( | Medium | 341 |
| `features.chat.manager` | `Exception` | raise AdapterError(f"LLM stream failed: {exc}") from exc | Medium | 363 |
| `features.chat.manager` | `ValueError/IndexError` | continue | Medium | 58 |
| `features.rag.handlers` | `Exception` | _logger.exception("Auto-save failed") | Medium | 133 |
| `features.rag.handlers` | `Exception` | _logger.exception("Delete chunks failed") | Medium | 194 |
| `features.rag.handlers` | `Exception` | _logger.exception("List namespaces failed") | Medium | 223 |
| `features.rag.handlers` | `Exception` | _logger.exception("Failed to save file") | Medium | 250 |
| `features.rag.handlers` | `Exception` | return { | Medium | 288 |
| `features.rag.handlers` | `Exception` | _logger.exception("Background reindex failed") | Medium | 331 |
